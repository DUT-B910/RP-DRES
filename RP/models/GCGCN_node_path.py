...
class GCGCN_node_path(nn.Module):
	def __init__(self, config):
		super(GCGCN_node_path, self).__init__()
		self.config = config
		print('model: GCGCN_node_path')
		print('原本的GCGCN模型，抽取最短路径，拼接路径上的结点表示进行预测,average pool')
	...
	def forward(self, document,document_ner,document_pos,adj_matrix,sen_matrix,pos_matrix_h,pos_matrix_t,node_pos,node_type,node_relative_pos,is_train,all_pairs_shortest_path):
		...
		for i in all_pairs_shortest_path.keys():
			for j in all_pairs_shortest_path[i].keys():
				if i ==j:
					...
					count_path += 1
					if len(all_pairs_shortest_path[i][j]) > max_path_len:
						max_path_len = len(all_pairs_shortest_path[i][j])

		entity_path_feature_dict= {}
		# path_before_rnn_feature = torch.zeros([count_path,max_path_len,node_feats_with_type.size(-1)]).cuda()
		...
		for i in all_pairs_shortest_path.keys():
			for j in all_pairs_shortest_path[i].keys():
				if i ==j:
					...
					path_list = all_pairs_shortest_path[i][j]
					origin_path_info = torch.cat([node_feats_with_type[i].unsqueeze(0) for i in path_list],dim = 0)	#[path_len*404]
					path_pad_list.append(origin_path_info)
					entity_path_feature_dict[count_pack] = (i , j)
					count_pack += 1

		padded_path = pad_sequence(path_pad_list)
		padded_path = padded_path.permute(1,0,2)
		# print('padded_path',padded_path.shape)	#路径数量 * 最大路径长度 *dim
		path_after_rnn = self.rnn_path(padded_path,max_path_len)
		...
		for k in range(max_path_len):
			i = entity_path_feature_dict[k][0]
			j = entity_path_feature_dict[k][1]
			entity_path_feature[i][j] = path_info_later[k] / torch.tensor(len(all_pairs_shortest_path[i][j]))
		# print('entity_path_feature',entity_path_feature.shape)
		entity_path_feature = torch.tanh(entity_path_feature)
		entity_path_feature = self.linear_path_class(entity_path_feature)
