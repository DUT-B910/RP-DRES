...
def init(data_file_name, rel2id, max_length = 512, is_training = True, suffix=''):
	...
	# for i_t in range(1):			#for debug
		...
		# print('len vertextset',len(vertexSet))
		# print('vertextset',vertexSet)
		...
		for j in range(len(vertexSet)):       #遍历实体
			...
			for k in range(len(vertexSet[j])):              #遍历第J个实体的所有mention
				...
				# print('sent_id',int(vertexSet[j][k]['sent_id']))
		...
		#4月2日 添加数据预处理，解析实体对之间的所有简单路径
		all_paths_list = []
		node_pairs_path_dict = {}

		node_num = len(articleGraph.nodes())
		count_simple_paths = 0
		...
		for i in range(node_num):
			for j in range(node_num):
				if i == j:
					...
					path = list(nx.all_simple_paths(articleGraph, source=i, target=j, cutoff=2))       #只抽取长度不超过cutoff的路径
					if len(path) ==0:
						continue
					else:
						start_index = count_simple_paths
						count_simple_paths += len(path)
						all_paths_list = all_paths_list + path
						# print(path)		#例如：[[14, 0, 12], [14, 12], [14, 13, 12]]
						end_index = count_simple_paths
						node_pairs_path_dict[(i,j)] = [start_index,end_index]

		item['node_pairs_path_dict'] = node_pairs_path_dict
		item['all_paths_list'] = all_paths_list
		# print('len node_pairs_path_dict',len(node_pairs_path_dict))
		# print(node_pairs_path_dict)
		# print('len all_paths_list',len(all_paths_list))	
		# print('count_simple_paths',count_simple_paths)
...
##########以下三行用于保存
		if (i_t+1)%storage_size ==0 or i_t == len(ori_data)-1:
			...
			print(os.path.join(out_path, name_prefix + suffix + str(int(i_t/storage_size))))
			data = []
