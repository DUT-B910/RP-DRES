...
class Config(object):
	def __init__(self, args):
		...
		self.test_epoch = 2
		self.pretrain_model = self.checkpoint_dir + '/GCN_node_all_path_node_edge_node_checkpoint_avgpooling_demo'
		# self.pretrain_model = None
		self.rank_path =None # self.result_dir + '/GraphCNN_multihead_bert_afterpretrain_hop2_rank_checkpointdev_train_rank.pkl'
	...
	def from_list_to_tensor(self,item,train = True):
		...
		#获得节点对的所有简单路径
		new_item['node_pairs_path_dict']=item['node_pairs_path_dict']
		new_item['all_paths_list'] = item['all_paths_list']
	...
	def load_train_data(self):
		...
		self.train_file = []
		self.train_file = pickle.load(open('./prepro_data_bert/new/dev_train0.pkl','rb'))
	...
	def load_test_data(self):
		...
		self.test_file = []  
		if prefix == 'dev_dev':
			self.test_file = pickle.load(open('./prepro_data_bert/new/dev_dev0.pkl','rb'))
			print("****************Finish reading dev_dev data***************")
		elif prefix == 'dev_test':
			self.test_file = pickle.load(open('./prepro_data_bert/new/dev_test0.pkl','rb'))
	...
	def gen_train_facts_distant(self):		
		data_file_name = './train_distant.json'
		fact_in_train = set([])     #创建一个无序不重复数据集合
		ori_data = json.load(open(data_file_name))
		for data in ori_data:
			vertexSet = data['vertexSet']
	...
	def train(self, model_pattern, model_name):
		...
		optimizer = optim.AdamW([
			{'params': model.bert.parameters(), 'lr': self.learn_rate * 0.01},
			{'params': base_params, 'weight_decay': 0.0001}
		], lr=self.learn_rate)
		...
		for epoch in range(self.max_epoch):
			...
			for i_data in tqdm(range(len(self.train_file))):   #一个batch的数据
				...
				node_pairs_path_dict = data['node_pairs_path_dict']
				all_paths_list = data['all_paths_list']
				predict_re = model(document,document_ner,document_pos,adj_matrix, \
									sen_matrix,pos_matrix_h,pos_matrix_t,node_pos,node_type, \
									node_relative_pos,True,node_pairs_path_dict,all_paths_list)
				...
				output = torch.argmax(predict_re, dim=-1)		#4.7修改
			...
			if f1 > best_f1 and f1 < self.max_f1 :
				best_f1 = f1
				best_auc = auc
				best_epoch = epoch
				best_ign_f1 = ign_f1
	...
	def test(self, model, model_name, output=False, input_theta=-1):
		...
		for data in tqdm(self.test_file):   #一个batch的数据
			...
			all_paths_list = data['all_paths_list']
			with torch.no_grad():
				predict_re = model(document,document_ner,document_pos,adj_matrix,sen_matrix,pos_matrix_h,pos_matrix_t,node_pos,node_type,node_relative_pos,False,node_pairs_path_dict,all_paths_list)

			predict_re = torch.sigmoid(predict_re)		#4.7添加
